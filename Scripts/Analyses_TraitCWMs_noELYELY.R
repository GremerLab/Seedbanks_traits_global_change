...
traitdat = read.csv("Data/Traits_PCscores_all_noELYELY.csv") #see Analyses_TraitPCAOutput_noELYELY.R which generates this file
...
dim(cwmdat) #90 plots
str(cwmdat)
#write.csv(cwmdat, "Data/CWMs_traits_PCs_noELYELY.csv")
    ...
    model_output1 = as.data.frame(m1$coefficients$fixed) %>%
      ...
      rename(Est = "m1$coefficients$fixed")
          ...
          if(round(anova_output1$'p-value'[5],3) >0.05 & round(anova_output1$'p-value'[6],3) >0.05 & round(anova_output1$'p-value'[7],3) >0.05){ #condition 2
            ...
            write.csv(anova_output2, file = paste("Output tables/Output_noELYELY/",response,"_main_both.csv",sep="")) 
            write.csv(model_output2, file = paste("Output tables/Output_noELYELY/",response,"_main_both_est.csv",sep="")) 
            kableExtra::save_kable(anova_output2_table, file = paste("Output tables/Output_noELYELY/",response,"_main_both.html",sep=""))      
            } else {
            #output results for 2 ways if they are significant 
              output_ind[[i]]= anova_output1
              mod_est_ind[[i]]= model_output1
               write.csv(anova_output1, file = paste("Output tables/Output_noELYELY/",response,"_2way_both.csv",sep=""))
               write.csv(model_output1, file = paste("Output tables/Output_noELYELY/",response,"_2way_both_est.csv",sep="")) 
            kableExtra::save_kable(anova_output1_table, file = paste("Output tables/Output_noELYELY/",response,"_2way_both.html",sep="")) }     
  } else { #output results for 3 way if it is significant 
    output_ind[[i]]= anova_output0
    mod_est_ind[[i]]= model_output0
    write.csv(anova_output0, file = paste("Output tables/Output_noELYELY/",response,"_3way_both.csv",sep=""))
    write.csv(model_output0, file = paste("Output tables/Output_noELYELY/",response,"_3way_both_est.csv",sep="")) 
    kableExtra::save_kable(anova_output0_table, file = paste("Output tables/Output_noELYELY/",response,"_3way_both.html",sep=""))
  }
...
#write.csv(all_anova_indtraits, file = "Output tables/Output_noELYELY/CWM_indtraits_ANOVA_all.csv")
...
#write.csv(all_anova_indtraits_wider2, file = "Output tables/Output_noELYELY/CWM_indtraits_ANOVA_all_wide.csv")
...
for (i in 1:length(pctraitlist)){
  ...
  if(round(anova_output0$'p-value'[anova_output0$factor == "habitat:watering:fertilization"],3) >0.05){ #condition 1
    ...
    write.csv(anova_output0, file = paste("Output tables/Output_noELYELY/",response,"_2way_both.csv",sep="")) 
    kableExtra::save_kable(anova_output0_table, file = paste("Output tables/Output_noELYELY/",response,"_3way_both.html",sep=""))
  }
...
all_anova_pctraits = do.call(rbind.data.frame, output_pc) %>%
  ...
  mutate(p = ifelse(round(p,3) == 0, "<0.001",round(p, 3))) %>%
  mutate(Fval = round(Fval, 3))
#write.csv(all_anova_pctraits, file = "Output tables/Output_noELYELY/CWM_pctraits_ANOVA_all.csv")
...
#write.csv(all_anova_pctraits_wider2, file = "Output tables/Output_noELYELY/CWM_pctraits_ANOVA_all_wide.csv")
